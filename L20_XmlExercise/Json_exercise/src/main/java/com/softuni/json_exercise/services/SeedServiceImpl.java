...
import com.softuni.json_exercise.domain.dtos.categories.wrappers.CategoryImportWrapperDto;
import com.softuni.json_exercise.domain.dtos.products.wrappers.ProductsImportWrapperDto;
import com.softuni.json_exercise.domain.dtos.users.wrappers.UsersImportWrapperDto;
...
public class SeedServiceImpl implements SeedService {
    ...
    public void seedUsers() throws IOException, JAXBException {
        if (this.userRepository.count() == 0) {
            ...
            final FileReader fileReaderXML = new FileReader(USER_XML_PATH.toFile());
...
//            fileReader.close();
            ...
            final UsersImportWrapperDto usersImportWrapperDto = (UsersImportWrapperDto) unmarshaller.unmarshal(fileReaderXML);

            List<User> users = usersImportWrapperDto.getUsers().stream()
...
//            fileReader.close();
            ...
            final FileReader fileReaderXML = new FileReader(CATEGORY_XML_PATH.toFile());

            final JAXBContext context = JAXBContext.newInstance(CategoryImportWrapperDto.class);
            final Unmarshaller unmarshaller = context.createUnmarshaller();
            final CategoryImportWrapperDto categoryImportWrapperDto = (CategoryImportWrapperDto) unmarshaller.unmarshal(fileReaderXML);

            List<Category> categories = categoryImportWrapperDto.getCategories().stream()
                    .map(categoryImportDto -> MODEL_MAPPER.map(categoryImportDto, Category.class))
                    .collect(Collectors.toList());
...
//            this.productRepository.saveAllAndFlush(productsList);
            ...
            final FileReader fileReaderXML = new FileReader(PRODUCT_XML_PATH.toFile());

            final JAXBContext context = JAXBContext.newInstance(ProductsImportWrapperDto.class);
            final Unmarshaller unmarshaller = context.createUnmarshaller();
            final ProductsImportWrapperDto productsImportWrapperDto = (ProductsImportWrapperDto) unmarshaller.unmarshal(fileReaderXML);

            List<Product> products = productsImportWrapperDto.getProducts().stream()
                    .map(productImportDto -> MODEL_MAPPER.map(productImportDto, Product.class))
                    .map(this::setRandomSeller)
                    .map(this::setRandomBuyer)
                    .map(this::setRandomCategories)
